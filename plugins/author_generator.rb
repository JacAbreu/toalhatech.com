...
module Jekyll
    ...
    def write_author_indexes
      if self.layouts.key? 'author_index'
        ...
        self.posts.each do |post|
          post_authors_dir = post.data["author"] || "noauthor"
          post_authors = post.data["author"]
          post_authors_dir = post_authors_dir.gsub(/\s+/, "")
          post_titles = post.data["title"]
          #linha abaixo estava comentada por mim.
          #post_authors.each do |author| 
          print " /titulo post #{post.data["title"]} "
                  
            self.write_author_index(File.join(dir, post_authors_dir), post_authors)
          #end
        end
  ...
  module Filters
    ...
    def author_links(authors)
      ...
      authors = authors.gsub(/\s+/, "")
        "<a class='author' href='/#{dir}/#{authors.downcase}/'>#{authors}</a>"
      #end
      case authors.length
      when 0
        ""
